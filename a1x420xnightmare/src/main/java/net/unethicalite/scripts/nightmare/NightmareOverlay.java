...
import net.unethicalite.scripts.nightmare.paint.CustomPaint;
import net.unethicalite.scripts.nightmare.paint.PaintInfo;
...
@Singleton
@Slf4j
class NightmareOverlay extends Overlay implements PaintInfo
{
	...
	public String[] getPaintInfo()
	{
		...
		if (!plugin.isInFight()) {
			return new String[] {
					...
					"Current tick:"+ API.ticks,
					"Loot gained:"+ API.convertToRSUnits(plugin.getTotalLootValue()),
					"Kill count:"+ plugin.getKillCount(),
					"In Fight? no",
					"Have gear for next fight? "+(plugin.isFulfilledEquipment() ? "YES!" : "no"),
					"Have swap gear for next fight? "+(plugin.isFulfilledGear() ? "YES!" : "no"),
					"Have supplies for next fight? "+(plugin.isHaveEnoughDosesForAnotherFight() ? "YES!" : "no"),
					"In Lobby? "+(plugin.isInLobby() ? "YES!" : "no"),
			};
		}
		return new String[] {
				"Phosani murderer"+version+" by Dreambotter420 ^_^",
	...
	public void renderScriptPaint(Graphics2D graphics2D)
	{
	...
	{
		renderScriptPaint(graphics);
		drawWalkTile(graphics);
	...
	private void drawWalkTile(Graphics2D graphics) {
		for (WorldPoint p : plugin.getWalkTile())
		{
			if (p == null) continue;
			Item i = new Item(1,1);

			p.outline(client, graphics, Color.GREEN, "");
		}
	}
