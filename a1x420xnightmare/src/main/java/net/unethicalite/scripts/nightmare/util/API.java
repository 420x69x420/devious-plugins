...
import net.unethicalite.scripts.nightmare.util.loadouts.Jewelry;
    ...
    public static void enablePrayer(boolean cursed, Prayer prayerIcon) {
        ...
        if (cursed) {
            if (prayerIcon.equals(Prayer.PROTECT_FROM_MAGIC) && (overhead == null || overhead != Prayer.PROTECT_FROM_MISSILES)) {
                log.info("[CURSED RANGED ATTACK] switching prayer from: "+(overhead == null ? "off" : overhead.toString()) + " to " + prayerIcon);
                ...
                log.info("[CURSED MAGIC ATTACK] switching prayer from: "+(overhead == null ? "off" : overhead.toString()) + " to " + prayerIcon);
                Prayers.toggle(prayerIcon);
                fastSleep();
            }  else if (prayerIcon.equals(Prayer.PROTECT_FROM_MISSILES) && (overhead == null || overhead != Prayer.PROTECT_FROM_MELEE)) {
                log.info("[CURSED MELEE ATTACK] switching prayer from: "+(overhead == null ? "off" : overhead.toString()) + " to " + prayerIcon);
                Prayers.toggle(prayerIcon);
                fastSleep();
            }
        } else if (overhead == null || overhead != prayerIcon) {
            log.info("[ATTACK] switching prayer from: "+(overhead == null ? "off" : overhead.toString()) + " to " +prayerIcon);
            Prayers.toggle(prayerIcon);
            fastSleep();
        }
    }
