...
import net.unethicalite.scripts.nightmare.util.API;
import net.unethicalite.scripts.nightmare.util.CombatStuff;
import net.unethicalite.scripts.nightmare.util.Doses;
import net.unethicalite.scripts.nightmare.util.HopWorld6HLog;
import net.unethicalite.scripts.nightmare.util.loadouts.*;
...
public class NightmarePlugin extends Script {
    ...
    private static boolean bankCached  = false;
    ...
    private boolean turnOffDefensivePrayers() {
        for (Prayer p : prayersToDefend) {
            if (Prayers.isEnabled(p)) {
                log.info("Turning off prayer: "+p.toString());
                Prayers.toggle(p);
                API.sleepClientTick();
                return true;
            }
    ...
    protected int loop() {
        ...
        if (SENGA_POOL_UNDERGROUND_AREA.contains(Players.getLocal())) {
            ...
            if (dialogSpriteText != null && dialogSpriteText.isVisible() && dialogSpriteText.getText().contains("You need 60,000 x Coins to retrieve your items.")) {
                log.info("Poor as fuck buddy, sorry mate, can't continue script, need 60k gp for dying noob ass recovery");
                this.stop();
                return -1;
            }
            ...
            if (dialogNPCText != null && dialogNPCText.isVisible() && dialogNPCText.getText().contains("Sorry, but I don't have anything for you. If I did")) {
                log.info("Collected all items from Senga! Nice");
        ...
        if (inFight) {
            ...
            if (nm == null || !nm.hasAction("Attack")) {
                ...
                if (!haveEnoughDosesForAnotherFight) {
                    ...
                    if (nmDiedTimer == null) {
                        log.info("Teleporting out due to not more doses for another fight");
            ...
            } else if (pendingNightmareAttack != null) {
            ...
            if (prepareAfterSleepwalkers) {
                ...
                if (!parasites.isEmpty()) {
                    ...
                    if (closestParasite == null || closestParasite.getName() == null) {
                        log.info("null NPC found of tracked PARASITES");
            ...
            if (huskSwitchTicks >= 1) {
                ...
                if (ourCurrentTarget != null && ourCurrentTarget.getName() != null) {
                    log.info("reset attack ticks for husk preparation by walking our own tile");
            ...
            if (whitelist.isEmpty()) {
                ...
                log.info("NOTHING IS SAFE!!! BLACKLIST TILE AND NO WHITELISTED");
            } else {
            ...
            if (!sortedWhitelist.get(0).equals(Players.getLocal().getWorldLocation())) {
                log.info("Hopping off unsafe tile");
                ...
                if (interactableNm != null && totemNPC == null) {
                    List<WorldPoint> finalInteractableNm = new ArrayList<>(interactableNm);
                    ...
                    if (interactableWalkableWhitelist.isEmpty()) {
                        ...
                        if (walkableWhitelist.isEmpty()) {
                            ...
                            log.info("Uhhh NO safetiles around nightmare :o");
                            return API.returnTick();
                        }
            ...
            if (ticksUntilParasite > 0 && ticksUntilParasite <= 3) {
                ...
                if (ourCurrentTarget != null && ourCurrentTarget.getName() != null) {
                    log.info("reset attack ticks for parasite preparation by walking our own tile");
            ...
            if (!parasites.isEmpty()) {
                if (whitelist.isEmpty()) {
                    ...
                    log.info("Hop off unsafe tile for parasite future attack");
                    walk(sortedWhitelist.get(0));
                    return API.returnTick();
                }
                NPC closestParasite = parasites.stream()
            ...
            if (parasite && drinkTickTimer == 0) {
                ...
                if (sanfew == -1) {
                    log.info("NO SANFEW LEFT WITH PARASITE IMPREGNATED!");
                } else {
                    Item sanfewPot = Inventory.getFirst(sanfew);
                    if (sanfewPot == null) {
                        log.info("Discrepancy between contains and get item methods for sanfew serum");
                    } else {
                        log.info("Drink sanfew potion for impregnation");
            ...
            if (totemNPC != null) {
                ...
                if (sortedWhitelistWalk.isEmpty()) {
                    ...
                    log.info("AFK this tick due to on whitelisted tile but out of range of possible totems (flower power + shadows + player in opposite corner of targeted totem)");
                    return API.returnTick();
                } else {
                    ...
                    if (!closestSortedWhitelistWalk.equals(Players.getLocal().getWorldLocation())) {
                        if (API.getClosestAttackDistance(totemNPC, closestSortedWhitelistWalk) > 8) {
                            ...
                            if (closestDist < ourDist) {
                                log.info("hop off future unsafe tile for totems (out of range) closer towards Totem");
                                walk(closestSortedWhitelistWalk);
                            } else {
                                log.info("Chilling here AFK while totems out of our immediate range and no tiles to get us closer");
                            }
                        } else {
                            log.info("hop off future unsafe tile for totems (in range)");
                            walk(closestSortedWhitelistWalk);
                        }
                        return API.returnTick();
                    }
                ...
                log.info("totem interact Charge");
            ...
            if (nm != null && nm.getName() != null && !nm.isDead() && nm.hasAction("Attack")) {
                ...
                if (result == -1) {
                    log.info("Not found any safe tiles near nightmare Reachable.getInteractable() filtered from blacklist :o");
                } else {
                    return result;
                }
            }
        ...
        if (arenaTiles.contains(Players.getLocal().getWorldLocation())) {
            ...
            if (!haveEnoughDosesForAnotherFight) {
                ...
                log.info("Waiting for NM to die");
                return API.returnTick();
            }
        ...
        if (walkOfaShameHimHaha) {
            if (!Inventory.contains(ItemID.ECTOPHIAL)) {
                ...
                if (!Bank.contains(ItemID.ECTOPHIAL)) {
                    log.info("No more teleports back to NM! Exiting script");
                    this.stop();
                    return -1;
                }
            ...
            if (walkToPhosanisLobby()) {
                //collect items here
                log.info("Collecting items here");
                        ...
                        if (secondSplit.length > 1) {
                            ...
                            if (stacksLeft > 0) {
                                if (Inventory.isFull()) {
                                    ...
                                    log.info("No more room in inventory when trying to collect items from senga, and no more anglerfish! wtf");
                                    return API.returnTick();
                                }
                                log.info("Have stacks: "+stacksLeft);
                ...
                if (sisterSenga == null || sisterSenga.getName() == null) {
                    log.info("Sister senga is null and we are in lobby area! :o");
                    return -1;
                }
                sisterSenga.interact("Collect");
                return API.returnTick();
            }
            log.info("Walk of A-Shame-Him-Haha");
            return API.returnTick();
        }
        if (!fulfilledEquipment || !haveEnoughDosesForAnotherFight || !fulfilledGear || (!inLobby && !NIGHTMARE_UNDERGROUND_WALK_ZONE.contains(Players.getLocal()))) {
            ...
            if (!bankCached && Bank.isEmpty()) {
                return API.clickLocalBank();
            }
            // Get lowest duel charge
            if (existingDuelCharge == -1) {
                log.info("Detected that we have no more rings of dueling left, stopping script - need this for banking :o");
                this.stop();
            }
            ...
            if (isInFeroxEnclave()) {
                ...
                if (Skills.getBoostedLevel(Skill.PRAYER) < Skills.getLevel(Skill.PRAYER) || Skills.getBoostedLevel(Skill.HITPOINTS) < Skills.getLevel(Skill.HITPOINTS) || Movement.getRunEnergy() < 80) {
                    ...
                    log.info("Interact 'Drink' on ferox pool");
                ...
                if (Skills.getBoostedLevel(Skill.HITPOINTS) <= Skills.getLevel(Skill.HITPOINTS)) {
                    ...
                    if (!Inventory.contains(ItemID.ANGLERFISH)) {
                        ...
                        if (ANGLERFISH == null) {
                            log.info("All out of ANGLERFISH :-(");
                            this.stop();
                            return -1;
                        }
                ...
                if (!Movement.isStaminaBoosted()) {
                    ...
                    if (!Inventory.contains(ItemID.STAMINA_POTION1)) {
                        ...
                        if (stamina == null) {
                            log.info("All out of stamina(1) doses:-(");
                            this.stop();
                            return -1;
                        }
                ...
                if (!gear.fulfilled()) {
                    log.info("Gear loadout fulfill start");
                    if (!gear.fulfill()) {
                        log.info("Missing something from our required gearset, cannot continue killing da night mare :-( exiting script");
                        this.stop();
                        return API.shortReturn();
                    }
                    return API.shortReturn();
                ...
                if (!Inventory.contains(i -> i.getId() == ItemID.ECTOPHIAL && i.hasAction("Empty"))) {
                    log.info("Script error: no ectophial with fulfilled gear including it");
                    return API.shortReturn();
                }
                useNMTeleport();
                return API.shortReturn();
        ...
        if (walkToPhosanisLobby()) {
            ...
            if (nightmarePool == null) {
                log.info("Script error: In senga/nightmare pool zone but nightmare pool null");
                return API.returnTick();
            }
    ...
    public boolean walkToPhosanisLobby() {
        ...
        if (NIGHTMARE_UNDERGROUND_WALK_ZONE.contains(Players.getLocal())) {
            ...
            if (!inLobby) {
                if (hopAround != null) {
                    log.info("Hopping around player: " + hopAround.getName() + "distance from us: " + hopAround.distanceTo(Players.getLocal()));
			...
			/*if (!haveTalkedToSisterSenga()) {
				...
				if (senga == null) {
					log.info("Script error: In senga/nightmare pool zone but senga null");
					return API.shortReturn();
				}
				API.shortSleep();
        ...
        if (surroundingNPC.size() > 0) {
            ...
            if (directlyAttack) {
                ...
                if (spec) {
                    ...
                    if (specPercent >= 100 && !Equipment.contains(specWep)) {
                        log.info("switch spec weapon at spec >= 100");
                    ...
                    if (specPercent >= minSpec && Equipment.contains(specWep)) {
                        log.info("toggle spec at "+specPercent+"%");
                        Combat.toggleSpec();
                        return API.returnTick();
                    }
                }
            ...
            if (npcClosestTile != null) {
                ...
                if (((!sporesWorld.isEmpty() || !shadowsPoints.isEmpty()) && ((Movement.isRunEnabled() && npcClosestTile.distanceTo(Players.getLocal()) > 1) || npcClosestTile.distanceTo(Players.getLocal()) > 2))) {
                    log.info("No action this tick cos "+npc.getName()+" too far to walk in range to 1 tick");
                    return API.returnTick();
                }
                walk(npcClosestTile);
            }
    ...
    protected void startUp() {
        ...
        log.info("TESTING: ENABLE DEATHWALK ON START");
    ...
    private void onProjectileSpawned(ProjectileSpawned event) {
        ...
        switch (projectile.getId()) {
            ...
            case 1768: //4 projectiles coming to nm to signal end of phase, start of sleepwalker transition
                if (!prepareSleepwalkers) {
                    log.info("Preparing for sleepwalker spawn");
    ...
    public void onAnimationChanged(AnimationChanged event) {
        ...
        if (!(actor instanceof NPC)) {
            if (actor instanceof Player && actor.getName() != null && actor.getName().equals(Players.getLocal().getName()) && actor.getAnimation() == PLAYER_TELEPORT_ANIMATION) {
                log.info("Detected player teleporting - clearing arena tiles");
                arenaTiles.clear();
            }
            return;
        }
    ...
    private int getNotedID(int itemID) {
        ...
        if (c == null) {
            log.info("Not found item composition for itemID: "+itemID);
            return -1;
        }
    ...
    public void onItemContainerChanged(ItemContainerChanged event) {
        ...
        for (Map.Entry<Integer, Integer> i : LootTracker.onItemContainerChanged(event).entrySet()) {
            ...
            if (allLoot.contains(id) && !Bank.isOpen()) {
                ...
                log.info("Picked up epic loot: "+qty+"x "+name+" ("+pricePer+"gp per = "+totalPrice+"gp total)");
    ...
    public void onActorDeath(ActorDeath event) {
        ...
        if (event.getActor() instanceof NPC && event.getActor().getName() != null) {
            ...
            if (npc.getName() != null && npc.getName().equalsIgnoreCase("sleepwalker")) {
                if (sleepwalkers.contains(npc)) {
                    log.info("remove despawned sleepwalker tag: " + npc.getTag());
                    sleepwalkers.remove(npc);
                }
            }
    ...
    private void onGameStateChanged(GameStateChanged event) {
        ...
        if (gamestate == GameState.LOADING && inFight) {
            log.info("Detected GameState == LOADING during fight! Must be dead or teleporting, clearing arena tiles");
            arenaTiles.clear();
            reset();
        }
    }
    ...
    private void onGameTick(final GameTick event) {
        ...
        if (sleepwalkersSpawned > 0) {
            log.info("Sleepwalker spawned: "+sleepwalkersSpawned);
        }
    ...
    public void turnOffAllPrayers() {
        if (Prayers.anyActive()) {
            log.info("Turning off some active prayers");
            Prayers.disableAll();
        }
    }
    ...
    public void killSleepwalker() {
        ...
        if (closestSleepwalker == null) {
            log.info("null NPC found of tracked SLEEPWALKERS with action ATTACK");
            return;
        }
        ...
        log.info("interacted attack on sleepwalker");
    }
    ...
    public void killHusks(List<Integer> swapGear) {
        NPC closestHusk = husks.stream()
                ...
                .orElse(null);
        if (closestHusk == null) {
            log.info("null NPC found of tracked HUSKS");
            return;
        }
        prayMeleeOffensive();
        ...
        log.info("interacted attack on husk");
        attackedHusk = true;
    }
    ...
    public int killParasite(NPC closestParasite, List<Integer> swapGear) {
        ...
        if (closestParasite.isMoving() && (!flowers.isEmpty() || !shadowsPoints.isEmpty() || !sporesWorld.isEmpty())) {
            log.info("Ignoring MOVING parasite when spores/shadows/flowers! Fucker");
            return API.returnTick();
        }
        int result = safelyWalkToInteractAttackMeleeNPC(closestParasite, swapGear, false, 0, 0, null);
        if (result == -1) {
            log.info("No safetiles found around parasite :o");
            return API.returnTick();
        }
        return result;
    }
