...
public enum Course
{
	GNOME_COURSE(() -> true, ObstacleFactory.newInstance(true)
	...
	DRAYNOR_COURSE(() -> Skills.getLevel(Skill.AGILITY) >= 10, ObstacleFactory.newInstance(true)
	...
	VARROCK_COURSE(() -> Skills.getLevel(Skill.AGILITY) >= 30, ObstacleFactory.newInstance(true)
	...
	CANFIS_COURSE(() -> Skills.getLevel(Skill.AGILITY) >= 40 && API.completedPriestInPeril,ObstacleFactory.newInstance(true)
	...
	FALADOR_COURSE(() -> Skills.getLevel(Skill.AGILITY) >= 50, ObstacleFactory.newInstance(true)
	...
	SEERS_COURSE(() -> Skills.getLevel(Skill.AGILITY) >= 60, ObstacleFactory.newInstance(true)
	...
	POLLNIVNEACH_COURSE(() -> Skills.getLevel(Skill.AGILITY) >= 70, ObstacleFactory.newInstance(true)
	...
	RELLEKKA_COURSE(() -> Skills.getLevel(Skill.AGILITY) >= 80, ObstacleFactory.newInstance(true)
	...
	ARDY_COURSE(() -> Skills.getLevel(Skill.AGILITY) >= 90, ObstacleFactory.newInstance(true)
	...
	Course(BooleanSupplier requirement, Obstacle[] obstacles)
	{
	...
	public static Course getHighest()
	{
		Course highest;
		...
		{
			if (course == NEAREST) continue;
			if (course.hasRequirements()) {
				validCourses.add(course);
			}
		}

		// Sort validCourses based on the ordinal values, which effectively sorts them by level requirements
		validCourses.sort(Comparator.comparingInt(Course::ordinal));

		// The last element in sorted validCourses will be the highest-level course the player can access
		highest = validCourses.get(validCourses.size() - 1);

		return highest;
	}

	/**
	 * Returns the nearest distance obstacle's area center point for any obstacle in the course
	 * @param player
	 * @return
	 */
	...
	/**
	 * Returns the nearest obstacle in the course, the area center point of which is closest to local player
	 * @return
	 */
