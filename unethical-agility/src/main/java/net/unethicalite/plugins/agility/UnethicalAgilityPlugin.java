...
@PluginDescriptor(
		name = "UnethicalAgility",
		enabledByDefault = false
)
@Slf4j
@Extension
public class UnethicalAgilityPlugin extends Script
{
	@Inject
	...
	private void onGameTick(GameTick event)
	{
		...
		API.ticks++;
		if (Vars.getVarp(302) >= 61) {
			API.completedPriestInPeril = true;
		}
	}
	private boolean cachedBank = false;
	...
	protected int loop()
	{
		...
		if (config.hop() && HopWorld6HLog.shouldHop()) {
		...
		if (mark != null && obstacle.getArea().contains(mark.getTile()) && canPick(mark))
		{
			if (mark.getTile().getGroundItems().contains(mark)) //failsafe sometimes onItemDespawned doesn't capture mog despawn
			{
				...
				if (gold != null && canPick(gold))
				{
					...
					if (destination == null || !destination.equals(gold.getWorldLocation())) {
						gold.pickup();
						return -2;
					}
				}
				WorldPoint destination = Movement.getDestination();
				if (destination == null || !destination.equals(mark.getWorldLocation())) {
					mark.pickup();
					return -2;
				}
			}
		...
		try
		{
			...
			if (tightrope && t == null) {
				return TileObjects.getFirstSurrounding(obstacle.getLocation().getX(), obstacle.getLocation().getY(),
						obstacle.getLocation().getZ(), 3, o -> Arrays.stream(tightropeIDs).anyMatch(i -> i == o.getId())
				);
			}
			return t;
		}
	...
	{
		return tileItem != null && (!Inventory.isFull() || Inventory.contains(ItemID.MARK_OF_GRACE));
	}
}
