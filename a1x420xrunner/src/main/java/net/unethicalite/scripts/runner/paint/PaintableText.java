...
public class PaintableText extends JComponent {
    ...
    public void paintComponent(Graphics graphics) {
        if (infoToPaint != null) { //Flag to switch between printing multiple backgrounds, or just one. In this case, just one background.
            ...
            Rectangle textRectangle = fontMetrics.getStringBounds(textToPaint, graphics).getBounds();

            int textHeight = (int) textRectangle.getHeight();
            int totalHeight = textHeight + verticalPadding * 2; // Total height including padding
            int yTextCenter = yCoord - verticalPadding + totalHeight / 2; // Center y-coordinate for text

            // Position for drawing the text, considering that text is drawn from its baseline
            int textBaselinePosition = yTextCenter + fontMetrics.getAscent() / 2 - fontMetrics.getDescent() / 2;

            // Positioning the rectangle
            textRectangle.setLocation(xCoord - horizontalPadding + borderThickness, yCoord - verticalPadding + borderThickness);
            textRectangle.setSize((int) textRectangle.getWidth() + horizontalPadding * 2, totalHeight);

            graphics2D.setColor(backgroundColor);
            graphics2D.fill(textRectangle);

            graphics2D.setColor(fontColor);
            graphics2D.drawString(textToPaint, xCoord, textBaselinePosition + 1);
